...
test('Load another page using saved token', async ({ request }) => {
  ...
  try {
    const response = await request.post('http://202.88.237.201:9988/auth/permissions', {
      headers: {
        Authorization: `Bearer ${token}`,
      },
      data: {
         "permission_name": "Branch Access",
  "description": "Permission to access branch-related data"
      },
    });
